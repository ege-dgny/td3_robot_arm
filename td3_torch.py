...
class Agent:
    def __init__(self, actor_learning_rate, critic_learning_rate, input_dims, tau, env, gamma=0.99, update_actor_interval=2,
                 ...
                 batch_size=100, noise=0.1, model_dir='tmp/td3'):
        self.gamma = gamma
        self.tau = tau
        ...
        # Create the network
        self.actor = ActorNetwork(input_dims=input_dims, fc1_dim=layer1_size, fc2_dim=layer2_size,
                                  n_actions = n_actions, name="actor", learning_rate=actor_learning_rate)

        self.critic_1 = CriticNetwork(input_dims=input_dims, fc1_dim=layer1_size, fc2_dim=layer2_size,
                                  n_actions = n_actions, name="critic_1", learning_rate=critic_learning_rate)

        self.critic_2 = CriticNetwork(input_dims=input_dims, fc1_dim=layer1_size, fc2_dim=layer2_size,
                                      n_actions=n_actions, name="critic_2", learning_rate=critic_learning_rate)

        #target network
        self.target_actor = ActorNetwork(input_dims=input_dims, fc1_dim=layer1_size, fc2_dim=layer2_size,
                                  n_actions=n_actions, name="target_actor", learning_rate=actor_learning_rate)

        self.target_critic_1 = CriticNetwork(input_dims=input_dims, fc1_dim=layer1_size, fc2_dim=layer2_size,
                                      n_actions=n_actions, name="target_critic_1", learning_rate=critic_learning_rate)

        self.target_critic_2 = CriticNetwork(input_dims=input_dims, fc1_dim=layer1_size, fc2_dim=layer2_size,
    ...
    def choose_action(self, observation, validation=False):
        ...
        mu_prime = T.clamp(mu_prime, self.min_action[0], self.max_action[0]) #helps us to limit for values higher/lower than the acceptable
    ...
    def learn(self):
        if self.memory.memory_counter < self.batch_size * 10:
            return
        ...
        #q s are the anticipated values of states
        ...
        #we want to overcome overestimation bias in q learning, instead of taking one or the other take the minimum
        next_critic_value = T.min(next_q1, next_q2)

        #trying to show aproximation of reward states from current reward
        ...
        actor_loss = -T.mean(actor_q1_loss) #negative allows the gradient ascent
        actor_loss.backward()
    ...
    def save_models(self, model_dir='tmp/td3'):
    ...
    def load_models(self, model_dir='tmp/td3'):
        try:
